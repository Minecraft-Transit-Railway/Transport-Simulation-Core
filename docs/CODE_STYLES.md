...
> Conventions enforced across the Transport Simulation Core codebase. New rules land here as
> they're agreed.

These rules apply to **both** the Java backend (`src/main/java/org/mtr/core/...`) and the
Angular frontend in `website/` unless otherwise noted. Anything not listed here defers to the
project's compiler / linter (Java 21 `javac` warnings, Angular ESLint flat config in
...
- **Operation / message keys exchanged over the wire** — `lower_snake_case`
  ...
  [`OperationProcessor.java`](../src/main/java/org/mtr/core/servlet/OperationProcessor.java)).
...
- **No Spring** — wire dependencies through plain constructors. The HTTP layer is Jetty
  servlets registered manually in [`Main.java`](../src/main/java/org/mtr/core/Main.java).
...
- **Lombok is allowed and encouraged** where it cleans things up:
	- `@RequiredArgsConstructor` on classes whose constructor is just `this.x = x; this.y = y;`
	  over `final` fields (see
	  [`CachedResponse`](../src/main/java/org/mtr/core/servlet/CachedResponse.java) for the
	  canonical example).
	- `@Log4j2` instead of `Main.LOGGER.error("", e);` in any class that does its own logging
	  (see [`Webserver`](../src/main/java/org/mtr/core/servlet/Webserver.java),
	  [`ServletBase`](../src/main/java/org/mtr/core/servlet/ServletBase.java),
	  [`Simulator`](../src/main/java/org/mtr/core/simulation/Simulator.java),
	  [`FileLoader`](../src/main/java/org/mtr/core/simulation/FileLoader.java)). Use
	  `@Log4j2`, **not** `@Slf4j` — the project logs through Log4j2 directly.
	...
	- The schema-generated classes under
	  [`src/main/java/org/mtr/core/generated/`](../src/main/java/org/mtr/core/generated/) are
	  Lombok-free by design — they're produced by the `Generator` task in `buildSrc/`.
...
- The schema-generated classes under
  [`src/main/java/org/mtr/core/generated/`](../src/main/java/org/mtr/core/generated/) and
...
Anything under `src/main/java/org/mtr/core/generated/` and
...
- **Angular**: follow <https://angular.dev/style-guide> — standalone components, signals over
  ...
  `inject()` over constructor parameters, `OnPush` change detection by default, the new
  `@if` / `@for` / `@switch` control flow syntax instead of `*ngIf` / `*ngFor`.
...
- Build & development workflow: [`BUILD.md`](BUILD.md).
- Running and CLI usage: [`RUNNING.md`](RUNNING.md).
- JSON schema mini-language and code generation: [`SCHEMA.md`](SCHEMA.md).
- The user-facing project blurb stays in the top-level [`README.md`](../README.md). Do
  **not** add API references, build commands or package layouts there — they go in `docs/`.
...
- **TS import grouping.** The high-traffic services (`map-data`, `directions`,
  `client(s)`, `station`, `departures`) have been grouped per §2.6, but several
  less-touched components / services still bunch all imports into one block. Sweep when
  editing.
- **Remaining magic numbers in the frontend.** Several constants in
  `website/src/app/component/map/map.component.ts` (animation duration, arrow spacing,
  client image size / padding, hex chrome colours) should move into a new
  `website/src/app/utility/map.constants.ts` per §3.1. The polling-interval magic numbers
  have already been lifted into
  [`website/src/app/utility/refresh.constants.ts`](../website/src/app/utility/refresh.constants.ts).
- **Wider Lombok adoption.** §3.6 lists Lombok as encouraged, but only `Webserver`,
  `ServletBase`, `Simulator`, `FileLoader` and `CachedResponse` have been migrated so far.
  Apply `@Log4j2` / `@RequiredArgsConstructor` to other hand-written boilerplate as you
  touch each file.
- **Hardcoded `http://localhost:4200`** in
  `website/src/app/component/map/map.component.ts` — should switch on Angular's
  `isDevMode()` rather than embedding the dev-server URL.
