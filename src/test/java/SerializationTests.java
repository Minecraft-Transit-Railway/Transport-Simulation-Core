...
public class SerializationTests implements Utilities, TestUtilities {
	...
	public void stationSerialization() {
		final Station station = new Station(TestUtilities.randomTransportMode(), TestUtilities.getDefaultSimulator());
	...
	public void sidingSerialization() {
		...
		final double acceleration = RANDOM.nextDouble() * Train.MAX_ACCELERATION * 2;
		siding.setAcceleration(acceleration);
		Assertions.assertEquals(siding.transportMode.continuousMovement ? Train.MAX_ACCELERATION : Train.roundAcceleration(acceleration), TestUtilities.getJsonObjectFromData(siding).get("acceleration").getAsDouble());
		setTimeValue(siding);
		...
		if (RANDOM.nextInt(3) > 0) {
			...
			while (true) {
				...
				vehicleCars.add(new VehicleCar(
						...
						TestUtilities.randomDoubleArray(RANDOM.nextInt(3)),
						TestUtilities.randomDoubleArray(RANDOM.nextInt(3))
				));
