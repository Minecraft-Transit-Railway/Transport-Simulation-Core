			...
			legacyRailNode.iterateConnections(railNodeConnection -> {
				...
				if (oldRailType != null) {
					final Rail rail = switch (railType) {
						case PLATFORM -> Rail.newPlatformRail(startPosition, startAngle, endPosition, endAngle, verticalRadius == 0 ? Rail.Shape.QUADRATIC : Rail.Shape.TWO_RADII, Math.max(verticalRadius, 0), 0, 0, 0, styles, transportMode);
						case SIDING -> Rail.newSidingRail(startPosition, startAngle, endPosition, endAngle, verticalRadius == 0 ? Rail.Shape.QUADRATIC : Rail.Shape.TWO_RADII, Math.max(verticalRadius, 0), 0, 0, 0, styles, transportMode);
						case TURN_BACK -> Rail.newTurnBackRail(startPosition, startAngle, endPosition, endAngle, verticalRadius == 0 ? Rail.Shape.QUADRATIC : Rail.Shape.TWO_RADII, Math.max(verticalRadius, 0), 0, 0, 0, styles, transportMode);
						default -> {
							...
							yield Rail.newRail(
									...
									shape, Math.max(verticalRadius, 0), 0, 0, 0, styles, railType.speedLimitKilometersPerHour, oldRailType.speedLimitKilometersPerHour,
