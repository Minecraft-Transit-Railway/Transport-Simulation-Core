...
const UTILITIES = {
	...
	getDrawStationElement: (stationElement, color1, color2) => {
		...
		element.className = "route-station-name text";
		if (color1 != null) {
			element.innerHTML = `<span class="route-segment bottom" style="background-color: ${UTILITIES.convertColor(color1)}">&nbsp</span>`;
		}
		if (color2 != null) {
			element.innerHTML += `<span class="route-segment top" style="background-color: ${UTILITIES.convertColor(color2)}">&nbsp</span>`;
		}
		element.innerHTML += `<span class="station-circle"></span>`;
		element.appendChild(stationElement);
		return element;
	},
	getDrawLineElement: (icon, innerElement, color, htmlBefore) => {
		const element = document.createElement("div");
		element.className = "route-duration";
		element.innerHTML =
			`<span class="route-segment ${color == null ? "walk" : ""}" style="background-color: ${color == null ? 0 : UTILITIES.convertColor(color)}">&nbsp</span>` +
