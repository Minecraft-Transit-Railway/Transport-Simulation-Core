...
const DATA = {
	...
	parseMTR: result => {
		...
		const elementRoutes = document.getElementById("search-results-routes");
		elementRoutes.innerHTML = "";
		const elementStations = document.getElementById("search-results-stations");
		...
		routes.forEach(route => {
			...
			elementRoutes.append(ACTIONS.getRouteElement(route["id"], route["color"], route["name"], route["number"], route["type"], false, true, "search-route-" + route["id"], true));
		});
		...
		Object.keys(stations).forEach(stationId => {
			const element = ACTIONS.getStationElement(stations[stationId]["color"], stations[stationId]["name"], stationId, "search-station-" + stationId);
