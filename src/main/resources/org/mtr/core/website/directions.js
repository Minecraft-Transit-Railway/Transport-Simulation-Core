...
const DIRECTIONS = {
	onSearch: index => {
		...
		const searchBox = document.getElementById("directions-box-" + index);
		const search = searchBox.value.toLowerCase().replace(/\|/g, " ");
		document.getElementById("clear-directions-" + index + "-icon").innerText = search === "" ? "" : "clear";
		...
		for (const stationId in stations) {
			document.getElementById("directions-" + index + "-" + stationId).style.display = resultsStations.includes(stationId) ? "block" : "none";
		}

		document.getElementById("search-results-stations-" + index).style.maxHeight = (Math.max(window.innerHeight - 320, 64)) / 4 + "px";
	...
	onSelectStation: (index, stationId) => {
		...
		document.getElementById("clear-directions-" + index + "-icon").innerText = "clear";
		document.getElementById("directions-box-" + index).value = data["stations"][stationId]["name"].replace(/\|/g, " ");
		for (const stationId in data["stations"]) {
			document.getElementById("directions-" + index + "-" + stationId).style.display = "none";
		}
		if (index === 1) {
		...
		if (stationStart !== 0 && stationEnd !== 0) {
			document.getElementById("directions-result").style.display = "";
			document.getElementById("directions-result-route").innerHTML = `<div class="info-center"><span class="material-icons large">${stationStart === stationEnd ? "horizontal-rule" : "refresh"}</span></div>`;
			shouldFetch = true;
	...
	writeStationsInResult: index => {
		const data = DATA.json[SETTINGS.dimension];
		const elementDirectionsStations = document.getElementById("search-results-stations-" + index);
		elementDirectionsStations.innerHTML = "";
		for (const stationId in data["stations"]) {
			const element = document.createElement("div");
			element.innerText = data["stations"][stationId]["name"].replace(/\|/g, " ");
			element.id = "directions-" + index + "-" + stationId;
...
const FETCH_DATA = new FetchData(() => `${SETTINGS.url}route?dimension=${SETTINGS.dimension}&startStation=${stationStart}&endStation=${stationEnd}`, DATA_REFRESH_INTERVAL, false, () => shouldFetch && stationStart !== 0 && stationEnd !== 0 && stationStart !== stationEnd, data => {
	...
	const resultElement = document.getElementById("directions-result-route");
	...
	if (drawStartStation) {
		...
		stationElement.className = "route-station-name-inner";
		stationElement.innerHTML =
			`<div class="route-segment-name">${start["station"]["name"].replace(/\|/g, " ")}</div>` +
			`<div class="route-segment-time">${getTimeString(totalDuration + initialWait)}</div>`;
	...
	for (let i = 0; i < directions.length; i++) {
		...
		stationElement.className = "route-station-name-inner";
		stationElement.innerHTML =
			`<div class="route-segment-name">${direction["station"]["name"].replace(/\|/g, " ")}</div>` +
			`<div class="route-segment-time">${getTimeString(totalDuration + (nextColor == null ? 0 : getData(i + 1, "route", "wait")))}</div>`;
		...
		if (!hasRoute && nextColor != null && platform) {
			...
			resultElement.append(UTILITIES.getDrawLineElement(circular === "" ? "" : circular === "cw" ? "rotate_right" : "rotate_left", createTextElement((routeNumber ? routeNumber + " " : "") + getRouteDestination(nextColor, thisStationId, getData(i + 1, "route", "stations")[0])), nextColor, `<span class="platform-circle" style="background: ${UTILITIES.convertColor(nextColor)}">${platform}</span>`));
		}
	...
	infoElement.className = "info-center";
...
document.getElementById("directions-icon").onclick = () => {
