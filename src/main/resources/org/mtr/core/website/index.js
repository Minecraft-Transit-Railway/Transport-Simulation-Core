				...
				for (let i = 0; i < result.length; i++) {
					result[i].forEach(playerData => {
						...
						playerListElement.innerHTML +=
							...
							`<div class="player-route" style="display: ${noRoute ? "none" : ""}; border-left: 0.3em solid ${UTILITIES.convertColor(color)}">` +
							`<div class="arrival">` +
							`<span class="arrival-text">&nbsp;${number.replace(/\|/g, " ")} ${name.split("||")[0].replace(/\|/g, " ")}</span>` +
							`</div>` +
							`<div class="arrival">` +
							`<span class="material-icons small">${circular === "" ? "chevron_right" : circular === "cw" ? "rotate_right" : "rotate_left"}</span>` +
							`<span class="arrival-text">${destination.replace(/\|/g, " ")}</span>` +
							`</div>` +
							`</div>` +
							`</div>`;
					});
