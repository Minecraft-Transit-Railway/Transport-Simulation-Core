...
const ACTIONS = {
	onClickStation: id => {
		...
		document.getElementById("station-name").innerHTML = stationNameHtml;
		document.getElementById("station-coordinates").innerText = `(${Math.round(x)}, ${Math.round(z)})`;
		document.getElementById("station-zone").innerText = zone;
		document.getElementById("station-line").style.backgroundColor = UTILITIES.convertColor(color);
		document.getElementById("station-copy").onclick = event => {
			navigator.clipboard.writeText(`/tp ${x} ~ ${z}`).then();
			event.target.innerText = "check";
			setTimeout(() => event.target.innerText = "content_copy", 1000);
		};
		document.getElementById("station-directions-1").onclick = () => {
			DOCUMENT.clearPanes(true);
			document.getElementById("directions").style.display = "block";
			DIRECTIONS.onSelectStation(1, id);
			document.getElementById("directions-box-2").focus();
		};
		document.getElementById("station-directions-2").onclick = () => {
			DOCUMENT.clearPanes(true);
			document.getElementById("directions").style.display = "block";
			DIRECTIONS.onSelectStation(2, id);
			document.getElementById("directions-box-1").focus();
		};

		const stationRoutesElement = document.getElementById("station-routes");
			...
			if (!addedRouteIds.includes(routeId)) {
				for (const stationIndex in route["stations"]) {
					if (route["stations"][stationIndex].split("_")[0] === id) {
						...
						arrivalElement.id = "station-arrivals-" + routeId;
	...
	setupRouteTypeAndDimensionButtons: () => {
		...
		document.getElementById("settings-route-types").style.display = oneOrFewerTypes ? "none" : "";
		if (oneOrFewerTypes) {
		...
		for (const routeType of Object.keys(UTILITIES.routeTypes)) {
			const element = document.getElementById("settings-route-type-" + routeType);
		...
		const settingsDimensionsButtons = document.getElementById("settings-dimensions-buttons");
		...
		for (const dimensionIndex in DATA.json) {
			if (DATA.json[dimensionIndex]["routes"].length > 0) {
				...
				element.id = "toggle-dimension-icon-" + dimensionIndex;
		...
		document.getElementById("settings-dimensions").style.display = dimensionButtonCount <= 1 ? "none" : "";
	},
	...
	getRouteElement: (routeId, color, name, number, type, visible, showColor, elementId, forceClick) => {
		...
		element.innerHTML =
			...
			`<span class="${showColor ? "text" : "text-disabled"} material-icons tight">${UTILITIES.routeTypes[type]}</span>` +
			`<span class="${showColor ? "text" : "text-disabled"}">${number}${number ? " " : ""}${name}</span>`;
		return element;
	},
	...
	for (const {arrival, name, destination, circular, platform, route, color} of result) {
		...
		if (arrivalsHtml[color]["count"] < MAX_ARRIVALS) {
			arrivalsHtml[color]["html"] +=
				`<div class="arrival">` +
				`<span class="arrival-text left-align material-icons tight" style="width: ${circular === "" ? 0 : 5}%">${circular === "" ? "" : circular === "cw" ? "rotate_right" : "rotate_left"}</span>` +
				`<span class="arrival-text left-align" style="width: ${circular === "" ? 70 : 65}%">` +
				...
				`</span>` +
				`<span class="arrival-text" style="width: 10%">${platform}</span>` +
				`<span class="arrival-text right-align" style="width: 20%; text-align: right">${arrivalDifference < 0 ? "" : UTILITIES.formatTime(arrivalDifference)}</span>` +
				`</div>`;
			arrivalsHtml[color]["count"]++;
		}
	}
	for (const color in arrivalsHtml) {
		const arrivalElement = document.getElementById("station-arrivals-" + color);
...
const FETCH_DELAYS_DATA = new FetchData(() => SETTINGS.url + "delays", REFRESH_INTERVAL, true, () => SETTINGS.selectedRoutes.length > 0, result => {
	...
	for (const {name, number, destination, circular, delay, time, x, y, z} of data) {
		...
		element.innerHTML =
			`<span class="arrival-text material-icons small" style="width: 5%; ${hugeDelay ? "color: " + routeColor : ""}">${bigDelay ? "warning" : "warning_amber"}</span>` +
			`<span class="arrival-text" style="width: 15%">${delay > MAX_DELAY_THRESHOLD ? "10:00+" : UTILITIES.formatTime(delay / 20)}</span>` +
			`<span class="arrival-text material-icons tight delay-copy-${copyButtonText}" style="width: 5%">${delayCopyButton === copyButtonText ? "check" : "content_copy"}</span>` +
			`<span class="arrival-text" style="width: 55%">(${x}, ${y}, ${z})</span>` +
			`<span class="arrival-text right-align" style="width: 15%; text-align: right">${isRealtime ? "" : "-" + UTILITIES.formatTime((millis - time) / 1000)}</span>` +
			`<span class="arrival-text right-align material-icons small" style="width: 5%; text-align: right">${isRealtime ? "" : "history"}</span>`;
		element.onclick = () => {
			navigator.clipboard.writeText(`/tp ${x} ${y} ${z}`).then();
			Array.from(document.getElementsByClassName(`delay-copy-${copyButtonText}`)).forEach(copyButtonElement => copyButtonElement.innerText = "check");
			delayCopyButton = copyButtonText;
			setTimeout(() => {
				Array.from(document.getElementsByClassName(`delay-copy-${copyButtonText}`)).forEach(copyButtonElement => copyButtonElement.innerText = "content_copy");
				delayCopyButton = "";
			}, 1000);
		};
		...
		if (!(name in routeNameElementMap)) {
			...
			routeNameElementMap[name].className = "station-list";
	...
	const routeStationsTabElement = document.getElementById("route-stations-tab");
	const routeDelaysTabElement = document.getElementById("route-delays-tab");
	if (hasDelay) {
	...
	document.getElementById("route-info").style.maxHeight = window.innerHeight - 80 + "px";
});
...
const onClickLine = (routeId, color, forceClick) => {
	...
	DOCUMENT.onClearSearch(false);
	DOCUMENT.clearPanes(false);
	document.getElementById("route-stations-tab").style.display = "none";
	document.getElementById("route-delays-tab").style.display = "none";
	const data = DATA.json[SETTINGS.dimension];
	...
	if (shouldSelect) {
		...
		document.getElementById("route-name").innerHTML = routeNameHtml;
		document.getElementById("route-line").style.backgroundColor = UTILITIES.convertColor(color);
		...
		selectedRoutes.forEach(route => {
			...
			routeStationsElement.className = "station-list"
...
const onSelectDelaysTab = selectDelaysTab => {
	...
	document.getElementById("route-stations-tab").className = "material-icons clickable" + (selectedDelaysTab ? "" : " selected");
	document.getElementById("route-delays-tab").className = "material-icons clickable" + (selectedDelaysTab ? " selected" : "");
	document.getElementById("route-details").style.display = selectedDelaysTab ? "none" : "";
	document.getElementById("delays").style.display = selectedDelaysTab ? "" : "none";
};

let selectedDelaysTab = false;
let delayCopyButton = "";
document.getElementById("route-stations-tab").onclick = () => onSelectDelaysTab(false);
document.getElementById("route-delays-tab").onclick = () => onSelectDelaysTab(true);
